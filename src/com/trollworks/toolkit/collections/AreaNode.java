...
class AreaNode implements AreaObject {
     ...
     * @param leafNode {@code true} if this should be a leaf node, {@code false} if not.
     */
    ...
    private void addLeavesToList(List<AreaObject> list) {
     ...
     * @return {@code true} if the tree from this point down currently contains the object.
     */
    ...
    protected AreaNode insert(AreaObject obj) {
        ...
        while (true) {
            ...
            for (int i = 0; i < oldNode.mStorageCount; i++) {
                ...
                long      growthAmt  = (long) union.height * union.width - (long) rectToGrow.height * rectToGrow.width;
        ...
        while (leaf != null) {
            ...
            if (split != null) {
                ...
                if (leaf != null) {
                    ...
                    split.mParent = leaf;
                    split = leaf.mStorageCount > MAX_PER_NODE ? splitNode(leaf) : null;
                }
            }
        }
    ...
    protected AreaNode remove(AreaObject obj) {
        ...
        if (leafNode != null) {
            List<AreaObject> savedLeaves = new ArrayList<>();
            int              size;
     ...
     * @return The root node for where the change occurred.
     */
    protected AreaNode remove(List<AreaObject> list) {
        AreaNode         root          = this;
        Set<AreaNode>    leavesVisited = new HashSet<>();
        List<AreaObject> savedLeaves   = new ArrayList<>();
    ...
    /**
     * If {@code exactMatch} is {@code true}, appends only those objects that have the exact same
     * coordinates as {@code bounds} to {@code result}, otherwise, appends all objects that
     * intersect with {@code bounds}.
     *
     * @param bounds     The bounds to search with.
     * @param result     The list to add matches to. May not be {@code null}.
     * @param exactMatch {@code true} to match coordinates exactly, {@code false} to only require an
     *                   intersection.
     */
    protected void search(Rectangle bounds, List<AreaObject> result, boolean exactMatch) {
        for (int i = 0; i < mStorageCount; i++) {
    ...
    /**
     * Appends all objects that intersect with {@code bounds} and are instances of the target
     * class.
     *
     * @param bounds      The bounds to search with.
     * @param result      The list to add matches to. May not be {@code null}.
     ...
     */
    protected void search(Rectangle bounds, List<AreaObject> result, Class<? extends AreaObject> targetClass) {
        for (int i = 0; i < mStorageCount; i++) {
            Rectangle nodeBounds = mStorage[i].getBounds();
            if (bounds == null || bounds.intersects(nodeBounds)) {
                if (!mLeafNode) {
    ...
    /**
     * Appends all objects that intersect with the {@code location} to {@code result}.
     *
     * @param location The location to search with. May not be {@code null}.
     * @param result   The list to add matches to. May not be {@code null}.
     */
    protected void search(Point location, List<AreaObject> result) {
        for (int i = 0; i < mStorageCount; i++) {
    ...
    /**
     * Appends all objects that intersect with the {@code location} and are instances of the target
     * class to {@code result}.
     *
     * @param location    The location to search with. May not be {@code null}.
     * @param result      The list to add matches to. May not be {@code null}.
     * @param targetClass The class of object for which we're looking
     */
    protected void search(Point location, List<AreaObject> result, Class<? extends AreaObject> targetClass) {
        for (int i = 0; i < mStorageCount; i++) {
            if (mStorage[i].getBounds().contains(location)) {
                if (!mLeafNode) {
                    ((AreaNode) mStorage[i]).search(location, result);
     ...
     * @param exactMatch {@code true} to match coordinates exactly, {@code false} to only require an
     *                   intersection.
     * @return The number of objects that intersect with {@code bounds}. If {@code exactMatch} is
    ...
    private static AreaNode splitNode(AreaNode node) {
        ...
        int          largestWasted   = -1;
        AreaObject   firstShape      = node.mStorage[0];
        AreaObject   secondShape     = node.mStorage[1];
    ...
    private static int areaWasted(Rectangle first, Rectangle second) {
        if (first.width > 0 && first.height > 0 && second.width > 0 && second.height > 0) {
            long      combinedArea = (long) first.width * first.height + (long) second.width * second.height;
            Rectangle union        = first.union(second);
            long      unionArea    = (long) union.height * union.width;
            Rectangle overlap      = first.intersection(second);
            long      overlapArea  = (long) overlap.height * overlap.width;
